...
const ASSENT_CONTACT = {
  whatsappNumber: '5562991383079',
  instagramUrl: 'https://instagram.com/assentagencia',
  copyrightYear: '2025',
  cnpj: '00.000.000/0001-00',
  cityLabel: 'Feito com propósito em Goiânia, GO'
};

class AssentFooter extends HTMLElement {
  connectedCallback() {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => this._inject(), { once: true });
      return;
    }
  ...
  _inject() {
    ...
    if (document.querySelector('assent-footer-el')) return;

    const waText = this.getAttribute('wa-text')
      || 'Olá! Vim pelo site da Assent Agência e gostaria de saber mais sobre os serviços.';
    const waHref = `https://wa.me/${ASSENT_CONTACT.whatsappNumber}?text=${encodeURIComponent(waText)}`;
    ...
    if (!document.getElementById('assent-footer-styles')) {
      ...
      style.textContent = `
        ...
        assent-footer-el .ft-social svg {
          width: 16px;
          height: 16px;
        }
    ...
    footer.innerHTML = `
      <div class="ft-container">
        ...
        <div class="ft-bottom">
          <div class="ft-copy">
            © ${ASSENT_CONTACT.copyrightYear} ASSENT AGÊNCIA — Especialistas em crescimento de negócios locais.<br>
            <span>Todos os direitos reservados. CNPJ: ${ASSENT_CONTACT.cnpj}</span>
          </div>
          <div class="ft-badge">
            <div class="ft-badge-dot"></div>
            ${ASSENT_CONTACT.cityLabel}
          </div>
        </div>
      </div>
    `;
    ...
    if (!hideWa && !document.querySelector('.assent-wa-float')) {
...
if (!customElements.get('assent-footer')) {
